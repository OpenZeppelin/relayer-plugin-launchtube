...
import { LogInterceptor } from '../../src/logger';
...
describe('PluginAPI', () => {
  ...
  describe('_send', () => {
    ...
    it('should throw error if write fails', async () => {
      ...
      await expect(pluginAPI._send('test-relayer', 'sendTransaction', payload)).rejects.toThrow(
        'Failed to send message to relayer',
      );
    });
  });
